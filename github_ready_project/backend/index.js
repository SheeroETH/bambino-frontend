...
app.post('/api/generate', rateLimiter, async (req, res) => {
  try {
    ...
    // Read local bambino.png
    const imagePath = path.join(__dirname, 'bambino.png');
    if (!fs.existsSync(imagePath)) {
      return res.status(500).json({ error: 'Base image bambino.png not found on server' });
    }
    ...
    const resultUrl = Array.isArray(output) ? output[output.length - 1] : output;
